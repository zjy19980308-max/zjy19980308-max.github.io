/* ═══════════════════════════════════════════════════════════════
   lanyard.js —— 可拖拽的挂绳工牌
   【v7】从 v1 张竞元作品集/dist/assets/lanyard.js 复制（v1 原件不动）。改动：标志换 owner 的 Figma logo（opt.logoVB 视框高、opt.logoColor 颜色），织带字色换米色。维护：网站交互调整。
...
(function (root) {
  ...
  function logo(g, o, cx, cy, h) {
    ...
    g.save();
    var k = h / (o.logoVB || 149.866);      /* 标志视框高：v1 logo.svg 149.866，v7 Figma logo 49.19 */
    g.translate(cx - (h / 2), cy); g.scale(k, k);
    g.fillStyle = o.logoColor || '#0E0E14';
  ...
  function makeStrapTex() {
    ...
    g.fillStyle = 'rgba(238,214,200,.78)';
  ...
  function mount(hostSel, opt) {
    ...
    var o = {
      ...
      logoPath: opt.logoPath || '', logoVB: opt.logoVB, logoColor: opt.logoColor,
